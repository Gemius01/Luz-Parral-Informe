...
class ReporteController extends Controller
{
    ...
    public function generarReporte(Request $request)
    {
        ...
        $users = DB::select(DB::raw("select * FROM (SELECT calling_station_id, SUBSTRING_INDEX(user_ip, '.', 3) as nas_port_id, DATE_FORMAT(from_time, '%d-%m-%Y') as fecha_inicio, DATE_FORMAT(from_time, '%d-%m-%Y %H:%i') as fecha_comparacion, DATE_FORMAT(from_time, '%H:%i:%s') as hora_inicio, DATE_FORMAT(till_time, '%d-%m-%Y') as fecha_termino, DATE_FORMAT(till_time, '%H:%i:%s') as hora_termino, round(download/1000000, 1) as descarga, round(upload/1000000, 1) as subida, (hour(uptime ) * 60 + minute (uptime )) as sesion FROM hotspot.servidor WHERE from_time BETWEEN '".$fechaIn."' AND '".$fechaTer."') AS fecha, (SELECT mac, DATE_FORMAT(fecha, '%d-%m-%Y %H:%i') as fecha, tipo, CASE WHEN web = '' THEN 'http://www.google.cl/' ELSE web END AS web FROM hotspot.webtipo) AS mac, (select id, SUBSTRING_INDEX(ip, '.', 3) as ip,zona, localidad, comuna, servicio from hotspot.zonaswifi) as zonaswifi WHERE (fecha.calling_station_id=mac.mac and zonaswifi.ip = fecha.nas_port_id and fecha.fecha_comparacion = mac.fecha)"));
